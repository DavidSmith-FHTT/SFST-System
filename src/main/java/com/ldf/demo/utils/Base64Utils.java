...
public abstract class Base64Utils {
    ...
    public static String encodeImage(String imgUrl) throws IOException {
        ...
        fis.read(rs);
        fis.close();
        return encodeToString(rs);
    }

    public static void decodeImage(String sourceStr, String targetUrl, String format) throws IOException {
        byte[] decode = decodeFromString(sourceStr);

        ByteArrayInputStream bais = new ByteArrayInputStream(decode);
        BufferedImage bi1 = ImageIO.read(bais);
        File w2 = new File(targetUrl);// 可以是jpg,png,gif格式
